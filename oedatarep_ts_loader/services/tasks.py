...
def register_ts():
	...
	for record in records:
		...
		ts_object.bootstrap_instance(record.id)
				...
				if ts_object.get_record_files():
					if ts_object.filter_ts_files():
						...
						for entry in ts_object.ts_files_to_upload:
							...
							if bool(header_data):
								header_data["name"] = header_data["name"]+"_"+str(record.id).replace("-","")
								ts_guid = ts_object.post_ts_header_on_tsd(header_data, tsd_token)

								current_app.logger.debug("Record (id:%s) - TS_Guid: %s", record.id, ts_guid)

								# if bool(ts_guid):
								# 	ts_object.put_ts_guid_on_record_metadata(ts_guid)
							else:
								current_app.logger.info("Record (id:%s) - has no TS_Header", record.id)
							...
							if bool(ts_data):
								current_app.logger.debug("Record (id:%s) - TS_Data: %s", record.id, bool(ts_data))
								ts_object.post_ts_data_on_tsd(ts_data, tsd_token)

								if bool(ts_guid):
									ts_object.put_ts_guid_on_record_metadata(ts_guid)
							else:
								current_app.logger.info("Record (id:%s) - has no TS_data", record.id)
					else:
						current_app.logger.info("Record (id:%s) - does not have a valid TS pair files(csv,json)", record.id)
				else:
					current_app.logger.info("Record (id:%s) - has no files", record.id)
			else:
				current_app.logger.info("Record (id:%s) - has no links resource to files", record.id)
